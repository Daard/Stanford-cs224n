...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    normed = lambda x : x / np.sum(np.sqrt(x.T.dot(x)))

    x = np.apply_along_axis(normed, 1, x)
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    for k in range(K):
        ind = indices[k + 1]
        z = sigmoid(- np.dot(outputVectors[ind], predicted))
        cost -= np.log(-z)
        grad[ind] += predicted * z
        gradPred += outputVectors[ind] * z
